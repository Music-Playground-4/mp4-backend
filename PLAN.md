...
- **공연 자동 팀빌딩**: **보류(미진행 결정)**. 프론트에 화면(auto-team/lineup)은 있으나 백엔드는 개발하지 않기로 함. 추후 필요 시 재검토.
...
| 자동 팀빌딩 | Team/Slot/매칭% | 없음 | Phase 6 (보류 — 미진행) |
...
## Phase 6 — 공연 자동 팀빌딩 (보류 · 미진행 결정)

> 2026-07-18 결정: 개발하지 않는다. 프론트 화면(auto-team/lineup)은 mock으로 두거나 추후 재검토.
