...
// PATCH /api/concerts/posts/:id/applications/:appId — 지원 수락/거절 (공고 작성자만)
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    const userId = await getSessionUserId(req)
    if (!userId) return unauthorized()

    const app = await prisma.concertApplication.findUnique({
    ...
    const updated = await prisma.concertApplication.update({
      where: { id: params.appId },
    ...
    await createNotification({
      ...
      body: parsed.data.status === 'ACCEPTED'
        ? `"${app.post.title}" 공연 지원이 수락되었어요!`
        : `"${app.post.title}" 공연 지원이 아쉽게 거절되었어요.`,
      data: { postId: params.id, status: parsed.data.status },
    })

    return ok(updated, '처리되었습니다.')
  } catch (e) {
    console.error('[PATCH /concerts/posts/:id/applications/:appId]', e)
    return serverError()
  }
}
